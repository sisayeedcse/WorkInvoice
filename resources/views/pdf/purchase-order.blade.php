...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .doc-title-box { background: #f47c1c; color: #fff; padding: 10px 16px; border-radius: 8px; display: inline-block; text-align: center; }
    .doc-title  { font-size: 12pt; font-weight: bold; letter-spacing: 1px; display: block; }
    ...
    .items-table tbody .item-desc { font-size: 7.5pt; color: #6c757d; margin-top: 2px; }
    .text-right { text-align: right; }
    .text-center { text-align: center; }
    .fw-bold { font-weight: bold; }
    .totals-table { width: 220px; margin-left: auto; margin-bottom: 14px; border-collapse: collapse; }
    .totals-table td { padding: 5px 10px; font-size: 8.5pt; border-bottom: 1px solid #e2e8f0; }
    .totals-table .grand-row { background: #f47c1c; color: #fff; font-weight: bold; font-size: 10pt; }
    .totals-table .grand-row td { border: none; padding: 8px 10px; }
    .notes-section { margin-bottom: 12px; font-size: 8.5pt; }
...
<div class="page">
    <div class="header">
        ...
        <div class="header-right">
            <div class="doc-title-box">
                ...
                <span class="doc-number">{{ $purchaseOrder->po_number }}</span>
            </div>
        </div>
    </div>
        ...
        <div class="info-right">
            <div class="info-box accent" style="text-align:left;">
                <table style="font-size:8pt;width:100%;">
                    <tr><td class="info-label" style="padding-right:14px;">PO Date:</td><td class="fw-bold">{{ $purchaseOrder->date->format('d M Y') }}</td></tr>
                    @if($purchaseOrder->delivery_date)
                    <tr><td class="info-label" style="padding-right:14px;">Delivery By:</td><td class="fw-bold">{{ $purchaseOrder->delivery_date->format('d M Y') }}</td></tr>
                    @endif
                    <tr><td class="info-label" style="padding-right:14px;">Status:</td><td class="fw-bold" style="color:#f47c1c;">{{ strtoupper($purchaseOrder->status) }}</td></tr>
                    <tr><td class="info-label" style="padding-right:14px;">Prepared By:</td><td class="fw-bold">{{ $company['owner'] }}</td></tr>
                </table>
            </div>
        </div>
    </div>
    ...
    <table class="totals-table">
        <tr><td>Subtotal</td><td class="text-right">{{ $company['currency_symbol'] }} {{ number_format($purchaseOrder->subtotal, 2) }}</td></tr>
        @if($purchaseOrder->discount > 0)
        <tr style="color:#16a34a;"><td>Discount</td><td class="text-right">- {{ $company['currency_symbol'] }} {{ number_format($purchaseOrder->discount, 2) }}</td></tr>
        @endif
        @if($purchaseOrder->tax > 0)
        <tr><td>Tax ({{ $purchaseOrder->tax }}%)</td><td class="text-right">{{ $company['currency_symbol'] }} {{ number_format(($purchaseOrder->subtotal - $purchaseOrder->discount) * $purchaseOrder->tax / 100, 2) }}</td></tr>
        @endif
        <tr class="grand-row">
            <td>GRAND TOTAL</td>
            <td class="text-right fw-bold">{{ $company['currency_symbol'] }} {{ number_format($purchaseOrder->grand_total, 2) }}</td>
        </tr>
    </table>
    ...
    <div class="notes-section">
        ...
        <div class="notes-box">{{ $purchaseOrder->notes }}</div>
    </div>
    @endif
    ...
    <div class="notes-section">
        ...
        <div class="notes-box">{{ $purchaseOrder->terms }}</div>
    </div>
    @endif
