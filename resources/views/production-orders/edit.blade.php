...
@section('content')
    <div class="page-header">
        <div>
            <h1>Edit {{ $productionOrder->production_number }}</h1>
            <p class="text-muted mb-0" style="font-size:13px;">Update pending production details and material usage.</p>
        </div>
    </div>

    <div class="form-main">
        <div class="card">
            <div class="card-header"><i class="bi bi-pencil-square me-2" style="color:var(--accent);"></i>Production Details
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('production-orders.update', $productionOrder) }}">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-4">
                            ...
                            <select name="finished_item_id" class="form-select" required>
                                @foreach($finishedProducts as $product)
                                    <option value="{{ $product->id }}" {{ (int) old('finished_item_id', $productionOrder->finished_item_id) === $product->id ? 'selected' : '' }}>
                                        {{ $product->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-3">
                            ...
                            <input type="number" min="0.001" step="0.001" name="quantity_to_produce" class="form-control"
                                value="{{ old('quantity_to_produce', $productionOrder->quantity_to_produce) }}" required>
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Date</label>
                            <input type="date" name="production_date" class="form-control"
                                value="{{ old('production_date', $productionOrder->production_date?->format('Y-m-d')) }}"
                                required>
                        </div>
                        <div class="col-md-2 d-flex align-items-end">
                            <button type="button" id="addMaterialBtn" class="btn btn-outline-primary w-100">Add
                                Material</button>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="notes" class="form-control"
                            rows="3">{{ old('notes', $productionOrder->notes) }}</textarea>
                    </div>

                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-accent px-4">Update Order</button>
                        <a href="{{ route('production-orders.show', $productionOrder) }}"
    ...
    <template id="materialRowTemplate">
        <tr>
            ...
            <td><input type="number" min="0.001" step="0.001" class="form-control quantity-input"
...
@push('scripts')
    <script>
        ...
        const existingMaterials = @json(old('materials', $productionOrder->materials->map(fn($m) => ['item_id' => $m->item_id, 'quantity_required' => (float) $m->quantity_required])->values()));
        ...
        function addMaterialRow(data = null) {
            ...
            if (data) {
                ...
                row.querySelector('.available-stock').textContent = `${parseFloat(option.dataset.stock || 0).toFixed(3)} ${option.dataset.unit || ''}`;
                row.querySelector('.quantity-input').value = data.quantity_required;
            }
        }
        ...
        tableBody.addEventListener('change', function (e) {
            ...
            e.target.closest('tr').querySelector('.available-stock').textContent = `${parseFloat(option.dataset.stock || 0).toFixed(3)} ${option.dataset.unit || ''}`;
        });
        ...
        if (existingMaterials.length) {
            existingMaterials.forEach(m => addMaterialRow(m));
        } else {
            addMaterialRow();
        }
    </script>
