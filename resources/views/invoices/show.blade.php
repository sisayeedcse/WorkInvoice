...
@section('content')
    <div class="hero-panel mb-4">
        <div>
            <div class="hero-kicker"><i class="bi bi-receipt"></i> Invoice Record</div>
            <h1 class="hero-title">{{ $invoice->invoice_number }}</h1>
            <p class="hero-copy">Monitor invoice status, open balance, and recorded payments from one structured collections
                view.</p>
        </div>
    ...
    <div class="row g-3">
        <div class="col-12 col-lg-4">
                        ...
                        @if($invoice->customer->phone)
                            <div class="meta-item"><span class="meta-label">Phone</span>
                                <div class="meta-value"><a
                                        href="tel:{{ $invoice->customer->phone }}">{{ $invoice->customer->phone }}</a></div>
                        </div>@endif
                        @if($invoice->due_date)
                            <div class="meta-item"><span class="meta-label">Due Date</span>
                                <div
                                    class="meta-value {{ now() > $invoice->due_date && $invoice->balance > 0 ? 'text-danger' : '' }}">
            ...
            <!-- Totals -->
            <div class="card mb-3">
                <div class="card-header"><i class="bi bi-calculator me-2" style="color:var(--accent);"></i>Payment Summary
                </div>
                <div class="card-body" style="font-size:13.5px;">
                    <div class="d-flex justify-content-between mb-2"><span class="text-muted">Invoice Total</span><span
                            class="fw-semibold">AED {{ number_format($invoice->grand_total, 2) }}</span></div>
                    <div class="d-flex justify-content-between mb-2 text-success"><span>Paid</span><span
                            class="fw-semibold">AED {{ number_format($invoice->paid_amount, 2) }}</span></div>
                    <hr>
                    <div class="d-flex justify-content-between fw-bold" style="font-size:15px;">
                        <span>Balance Due</span>
                        <span class="{{ $invoice->balance > 0 ? 'text-danger' : 'text-success' }}">AED
                            {{ number_format($invoice->balance, 2) }}</span>
                    </div>
                </div>
            </div>
            ...
            @if($invoice->balance > 0)
                <div class="card">
                    <div class="card-header"><i class="bi bi-cash-coin me-2" style="color:var(--accent);"></i>Record Payment
                    </div>
                    <div class="card-body">
                        <form method="POST" action="{{ route('invoices.add-payment', $invoice) }}">
                            ...
                            <div class="mb-2">
                                ...
                                <input type="number" name="amount" class="form-control" step="0.01" min="0.01"
                                    max="{{ $invoice->balance }}" value="{{ number_format($invoice->balance, 2, '.', '') }}"
                                    required>
                            </div>
                            <div class="mb-2">
        ...
        <div class="col-12 col-lg-8">
            ...
            @if($invoice->payments->count())
                <div class="card">
                    <div class="card-header"><i class="bi bi-clock-history me-2" style="color:var(--accent);"></i>Payment
                        History</div>
                    <div class="table-responsive">
                        <table class="table mb-0">
                            <thead>
