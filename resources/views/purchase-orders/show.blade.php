...
@section('content')
    <div class="hero-panel mb-4">
        <div>
            <div class="hero-kicker"><i class="bi bi-cart-check"></i> Purchase Order Record</div>
            <h1 class="hero-title">{{ $purchaseOrder->po_number }}</h1>
            <p class="hero-copy">Review supplier details, purchasing totals, and receipt status from a cleaner procurement
                detail view.</p>
        </div>
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between mb-3">
                        <div>
                            ...
                            <div class="text-muted" style="font-size:12px;">{{ $purchaseOrder->date->format('d M Y') }}
                            </div>
                        </div>
                    ...
                    <form method="POST" action="{{ route('purchase-orders.update-status', $purchaseOrder) }}"
                        ...
                        <select name="status" class="form-select form-select-sm">
                            @foreach(['draft', 'sent', 'received', 'cancelled'] as $s)
                                <option value="{{ $s }}" {{ $purchaseOrder->status == $s ? 'selected' : '' }}>{{ ucfirst($s) }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="meta-list mt-3">
                        <div class="meta-item"><span class="meta-label">Supplier</span>
                            <div class="meta-value">{{ $purchaseOrder->supplier_name }}</div>
                        </div>
                        @if($purchaseOrder->supplier_phone)
                            <div class="meta-item"><span class="meta-label">Phone</span>
                                <div class="meta-value"><a
                                        href="tel:{{ $purchaseOrder->supplier_phone }}">{{ $purchaseOrder->supplier_phone }}</a>
                                </div>
                        </div>@endif
                        @if($purchaseOrder->supplier_email)
                            <div class="meta-item"><span class="meta-label">Email</span>
                                <div class="meta-value">{{ $purchaseOrder->supplier_email }}</div>
                        </div>@endif
                        @if($purchaseOrder->supplier_address)
                            <div class="meta-item"><span class="meta-label">Address</span>
                                <div class="meta-value">{{ $purchaseOrder->supplier_address }}</div>
                        </div>@endif
                    </div>
                </div>
            </div>
            ...
            <div class="card">
                ...
                <div class="card-body" style="font-size:13.5px;">
                    <div class="d-flex justify-content-between mb-2"><span class="text-muted">Subtotal</span><span>AED
                            {{ number_format($purchaseOrder->subtotal, 2) }}</span></div>
                    @if($purchaseOrder->discount > 0)
                        <div class="d-flex justify-content-between mb-2 text-success"><span>Discount</span><span>- AED
                    {{ number_format($purchaseOrder->discount, 2) }}</span></div>@endif
                    @if($purchaseOrder->tax > 0)
                        <div class="d-flex justify-content-between mb-2 text-muted"><span>Tax
                                ({{ $purchaseOrder->tax }}%)</span><span>AED
                                {{ number_format(($purchaseOrder->subtotal - $purchaseOrder->discount) * $purchaseOrder->tax / 100, 2) }}</span>
                    </div>@endif
                    <hr>
                ...
                <div class="card">
                    <div class="card-body">
                        @if($purchaseOrder->notes)
                            <div class="mb-3"><strong style="font-size:13px;">Notes</strong>
                                <div class="text-muted mt-1" style="font-size:13.5px;">{{ $purchaseOrder->notes }}</div>
                            </div>
                        @endif
                        @if($purchaseOrder->terms)
                            <div><strong style="font-size:13px;">Terms & Conditions</strong>
                                <div class="text-muted mt-1" style="font-size:13px;white-space:pre-line;">
                                    {{ $purchaseOrder->terms }}</div>
                            </div>
                        @endif
                    </div>
                </div>
