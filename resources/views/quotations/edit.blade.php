...
@php
    $formItems = old('items', $quotation->items->map(function ($item) {
        return [
...
<div class="qf-hero">
    <div class="qf-hero-left">
        <div class="qf-hero-badge mb-2">
            <i class="bi bi-pencil-square"></i> Editing Quotation
        </div>
        <h1>Edit {{ $quotation->quotation_number }}</h1>
        <p>Update customer details, dates, and line items — totals recalculate instantly.</p>
    </div>
...
<div class="qf-tip">
    ...
    <div><strong>Workflow tip:</strong> Save changes here, then use the quotation detail page to convert to Order or Invoice when approved.</div>
</div>
    ...
    <div class="qf-section-body">
        <div class="qf-details-grid">
            ...
            <div>
                ...
                <div class="qf-input-wrap">
                    ...
                    <input type="date" name="date" class="form-control"
                           value="{{ old('date', optional($quotation->date)->format('Y-m-d')) }}" required>
                </div>
            </div>
            <div>
            ...
            <div>
                ...
                <div class="qf-input-wrap">
                    <i class="bi bi-flag qf-input-icon"></i>
        ...
        <table class="qf-table" id="items-table">
            ...
            <tbody id="items-container">
                ...
                <tr class="item-row" data-index="{{ $i }}">
                    <td><div class="qf-row-num">{{ $loop->iteration }}</div></td>
                    <td><input type="text" name="items[{{ $i }}][item_name]"
                    ...
                    <td><input type="text" name="items[{{ $i }}][size]"
                               class="form-control"
                               value="{{ $item['size'] ?? '' }}"
                               placeholder="e.g. 2x3m"></td>
                    <td><input type="number" name="items[{{ $i }}][quantity]"
                               class="form-control qty-input text-center"
                               value="{{ $item['quantity'] ?? 1 }}" step="0.01" min="0.01" required></td>
                    <td><select name="items[{{ $i }}][unit]" class="form-select">
                        @foreach(['Unit','Piece','Meter','Sqm','Kg','Set','Lot','Hour','Day','Job'] as $u)
                        <option value="{{ $u }}" {{ ($item['unit'] ?? 'Unit') === $u ? 'selected' : '' }}>{{ $u }}</option>
                        @endforeach
                    </select></td>
                    <td><input type="number" name="items[{{ $i }}][unit_price]"
                               class="form-control price-input"
                               value="{{ $item['unit_price'] ?? 0 }}" step="0.01" min="0" required></td>
                    <td><div class="qf-row-total" id="row-total-{{ $i }}">0.00</div></td>
        ...
        <div class="row g-4">
            <div class="col-12 col-md-5">
                ...
                <textarea name="notes" class="form-control" rows="5"
                          ...
                          placeholder="e.g. Thank you for the opportunity. This quote is valid for 30 days.">{{ old('notes', $quotation->notes) }}</textarea>
            </div>
            <div class="col-12 col-md-7">
                <label class="qf-label">Terms &amp; Conditions</label>
                <textarea name="terms" class="form-control" rows="5"
                          style="font-size:.875rem; resize:vertical;"
                          placeholder="e.g. Payment due within 15 days of invoice. All prices are exclusive of VAT unless stated.">{{ old('terms', $quotation->terms) }}</textarea>
            </div>
        </div>
    </div>
    <div class="qf-actions">
        <button type="submit" class="btn-qf-save">
            <i class="bi bi-check2-circle"></i> Update Quotation
        </button>
        <a href="{{ route('quotations.show', $quotation) }}" class="btn-qf-cancel">
...
function addRow(name, desc, price, unit) {
    ...
    tr.innerHTML =
        '<td><div class="qf-row-num">' + (document.querySelectorAll('.item-row').length + 1) + '</div></td>' +
